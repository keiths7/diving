...
$.fn.kuiDate = function (k_date) {
    ...
    function kui_date() {
        ...
        var month_name_en = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'June', 'July', 'Aug', 'Sept', 'Oct', 'Nov', 'Dec'];
        $('.kui_today').text(month_name_en[Number(now_month) - 1] + ' ' + now_year);
        // 上月下月
        $('.kui_prev_m').click(function () {
            var kui_y = now_year;
            var kui_m = now_month;
            if (kui_m == 1) {
                now_year = kui_y - 1;
        ...
        function change_date(dir) {
            ...
            if (dir == 'right') {
                ...
                $('.kui_tomorrow').text(month_name_en[Number(kui_m) - 1] + ' ' + kui_y);
            } else {
            ...
            for (var i = 0; i < kui_td_lines; i++) {
                ...
                if (i == 0) {
                    ...
                    for (var j = bb + 1; j <= 7; j++) {
                        ...
                        arr_tr.push('<dt class="' + cla + '"  onclick="kui_click_(this,' + now_date + ',' + kui_y + ',' + kui_m + ',' + kui_d + ');">' + (7 * i + kui_i) + '</dt>');
                        kui_i++;
                    }
                    ...
                    for (var j = 1; j <= 7; j++) {
                        ...
                        if (7 * (i - 1) + kui_i > this_max_date) {
                            ...
                            arr_tr.push('<dt class="' + cla + '" onclick="kui_click_(this,' + now_date + ',' + kui_y + ',' + kui_m + ',' + kui_d + ');">' + (7 * (i - 1) + kui_i) + '</dt>');
                        }
                        kui_i++;
                    }
                } else {
                    ...
                    for (var j = 1; j <= 7; j++) {
                        ...
                        arr_tr.push('<dt class="' + cla + '"  onclick="kui_click_(this,' + now_date + ',' + kui_y + ',' + kui_m + ',' + kui_d + ',1);">' + (7 * (i - 1) + kui_i) + '</dt>');
                        kui_i++;
                    }
                }
            }
...
$(function () {
    ...
    //     // var data_pane = $('.kui_data_content_pane');
    //     //  var target=e.target;
    //     // var t_id = $(target).attr('t_id');
    //     // if (t_id == 'kui_date') {
    //     // } else if (!data_pane.is(target) && data_pane.has(target).length === 0) {
    //     //     // $('#kui_d_pane').hide();
    //     // }
    // });
});
$(function () {
    /* 日期插件的HTML元素 */
    var kui_div_date = '<div class="kui_d_pane" id="kui_d_pane" style="display:none;"><iframe id="kui_frame_d" width="370" height="203" frameborder="0"></iframe><div class="kui_data_content_pane"><div class="kui_prev_next_month"><i href="javascript:;" class="kui_prev_m  bordered  angle left icon"></i><span class="kui_today"></span><i href="javascript:;" class="kui_next_m bordered  angle right icon"></i><span class="kui_tomorrow"></span></div><div id="left_table"><dl class="kui_data_tab"><dt class="d_th_bg">Su</dt><dt>Mo</dt><dt>Tu</dt><dt>We</dt><dt>Th</dt><dt>Fr</dt><dt class="d_th_bg">Sa</dt></dl><dl class="kui_date_info" id="kui_left_t"></dl></div><div id="right_table"><dl class="kui_data_tab"><dt class="d_th_bg">Su</dt><dt>Mo</dt><dt>Tu</dt><dt>We</dt><dt>Th</dt><dt>Fr</dt><dt class="d_th_bg">Sa</dt></dl><dl class="kui_date_info" id="kui_right_t"></dl></div><div class="kui_date_reset"><span class="off">clear</span><span class="close">close</span></div></div></div>';
